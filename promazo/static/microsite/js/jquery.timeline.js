	...
	$.widget( "grantorino.timeline" , {
		...
		_tpl_event: ['<li class="tl-item">',
						'<div class="tl-wrap {{class}}">',
							...
							'<div class="tl-content panel padder b-a {{classname}}">',
								'<span class="arrow left pull-up {{classname}}"></span>',
								'<div>{{content}}</div>',
							'</div>',
						'</div>',
					'</li>'
		...
		_render_event: function(data){
            ...
            event_html = event_html.replace('{{classname}}', data.classname);
            event_html = event_html.replace('{{classname}}', data.classname);
		...
		_format_time: function(time){
			...
			return ( month + ' ' + day + '<br />' + hours + ':' + minutes + ' ' + ampm);
		},
